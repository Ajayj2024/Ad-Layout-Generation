...
class LayoutPlacementDataset(Dataset):
    def __init__(self, args, data_split):
        ...
        self.source_path = os.path.join('dataset', f'place_stage/{data_split}.json')
        # self.target_path = os.path.join('dataset', '')
    ...
    def build_json_data(self):
        ...
        self.source_data = [item['serialize_constraints'] for item in _data]
        self.target_data = [item['plain_layout_seq'] for item in _data]
        # if self.args.is_two_stage:
