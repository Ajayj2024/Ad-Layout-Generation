...
def get_element_types(subtree: LarkTree) -> defaultdict:
    ...
    for child in subtree.children:
        if ir_utils.is_element(child):
            etype = ir_utils.get_element_type(child)
            type_count[etype] += 1
    return type_count
...
class SqueezeItem(Squeeze):
    ...
    def _squeeze(self, pt: LarkTree) -> LarkTree:
        ...
        if group is None:
            return None
        group_items = [child for child in group.children if ir_utils.is_item(child)]
        ...
        group.children = [Token('GROUP_TYPE', ir_utils.get_group_type(group)),
                          ir_utils.create_group_attr('repeat', num_repeat),
                          template]
        return pt
...
class SqueezeMultiInfoItems(SqueezeItem):
    ...
    def _clean_values(self, item: LarkTree):
        for element in item.children:
            if ir_utils.is_element(element):
                value_attr = None
                for attr in element.children:
                    if ir_utils.is_target_attr(attr, 'value'):
                        value_attr = attr
                        break
...
class SqueezeMultiInfoItemsWithNames(SqueezeMultiInfoItems):
    ...
    def _get_values(self, items: List[LarkTree]) -> List[str]:
        ...
        for item in items:
            for element in item.children:
                if ir_utils.is_target_element(element, 'title'):
                    for attr in element.children:
                        if ir_utils.is_target_attr(attr, 'value'):
                            values.append(ir_utils.get_attr(attr).strip("'"))
        return values
    ...
    def _squeeze(self, pt: LarkTree) -> LarkTree:
        ...
        group.children = [Token('GROUP_TYPE', ir_utils.get_group_type(group)),
                          ir_utils.create_group_attr('repeat', num_repeat),
                          ir_utils.create_group_attr('names', ", ".join(names)),
                          template]
        return pt
    ...
    def _squeeze(self, pt: LarkTree) -> LarkTree:
        ...
        for item in group_items:
            for child in item.children:
                if ir_utils.is_target_element(child, 'link'):
                    total_links += 1

        group.children = [Token('GROUP_TYPE', ir_utils.get_group_type(group)),
                          ir_utils.create_group_attr('repeat', num_repeat),
                          ir_utils.create_group_attr('total_links', total_links)]
        return pt
...
class FuzzySqueezeFooterItemsWithNames(SqueezeFooterItemsWithTotalLinks):
    ...
    def _get_values(self, items: List[LarkTree]) -> List[str]:
        ...
        for item in items:
            for element in item.children:
                if ir_utils.is_target_element(element, 'text') or ir_utils.is_target_element(element, 'title'):
                    for attr in element.children:
                        if ir_utils.is_target_attr(attr, 'value'):
                            values.append(ir_utils.get_attr(attr).strip("'"))
        return values
...
class Synthesizer:
    ...
    def _create_element(self, etype: str, pos: str, size: str, value: str = None,
        ...
        if value is not None:
            if value.lower() in vital_values:
                ...
                if 1 <= num_words <= 5:
                    attrs.append(('value', f'{value}'))
        return ir_utils.create_element(etype, attrs)
    ...
    def _create_group(self, curr_parent: LarkTree, region_type: str) -> LarkTree:
        if curr_parent.data == 'region':
            ...
            return ir_utils.create_group(group_type)
        elif curr_parent.data == 'group':
            return ir_utils.create_item()
        else:
    ...
    def __call__(self, placement_input: str, element_info: List = None, vital_values: Set = None) -> List[LarkTree]:
        ...
        region = ir_utils.create_region(region_type)
...
if __name__ == "__main__":
    ...
    print(synthesizer("background 0 0 120 76 | background 0 0 120 66 | button 6 52 18 5 | button 26 52 19 5 | description 5 39 52 9 | image 58 33 60 37 | title 5 23 52 14"))